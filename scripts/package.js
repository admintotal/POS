...
const generarInstaladorWindows = async (platform) => {
    ...
    execSync(`rsync -av --progress ${localPath} ${paths.appBuild}/Admintotal/${platform}`, {stdio:[0,1,2]})
    log('info', `Instalador guardado en ${paths.appBuild}/Admintotal/${platform} `)
}
...
const comprimir = async (dirPath, outputPath) => {
    ...
    execSync(`pushd ${dirPath}; zip -r ../${outputPath} ./*; popd`)
  ...
  fetch(url, options).then((res) => {
    return res.json().then((json) => {
        ...
        if (json.status == 'success') {
            log('success', "Actualización registrada correctamente")
        } else {
    ...
    let proms = appPackageJson.nwBuilder.platforms.map(async (platform) => {
        ...
        try {
            ...
            if (isUnix) {
                ...
                generarInstaladorWindows(platform)
            }
        } 
        catch(err) {
